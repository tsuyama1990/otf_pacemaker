...
class MaxVolSampler(Sampler):
    ...
    def sample(self, **kwargs) -> List[Tuple[Atoms, int]]:
        ...
        # We assume the Orchestrator has handled symbol mapping or ASE can infer if properly set up.
        # However, read(format='lammps-dump-text') reads types as atomic numbers usually or just types.
        # We need to ensure symbols are correct for pace_select.
        # The Orchestrator usually runs `_ensure_chemical_symbols` on a single frame.
        # Here we need to do it for all frames.
        ...
        # Determine elements to map types to symbols
        # We need 'elements' list. Usually in config.
        # We can pass 'elements' in kwargs if needed, or assume frames have them if read differently.
        # LAMMPS dump usually doesn't have symbols.
        ...
        for atoms in selected_frames:
             # Find max gamma index
             # pace_select output should preserve the properties including gamma
             # check keys
             gamma_key = None
             possible_keys = ["f_f_gamma", "f_gamma", "gamma", "MaxVol_Gamma"] # pace_select might rename?
             # Usually pace_select preserves arrays if using extxyz
             ...
             if not gamma_key:
                  # If gamma is missing in arrays, check info?
                  # Or maybe recalculate? No, we should rely on what was in the dump.
                  # If extxyz conversion preserved it, it should be there.
